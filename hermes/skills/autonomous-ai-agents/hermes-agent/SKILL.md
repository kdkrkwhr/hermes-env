---
name: hermes-agent
description: "Configure, extend, or contribute to Hermes Agent — including multi-profile setups (separate bots/personas, per-profile skills and cron)."
version: 2.1.0
...
For committing skills under the hermes-agent package tree (frontmatter validator, directory layout), see **`references/hermes-agent-skill-authoring.md`**. User-local skills still use `skill_manage(action='create')`.
...
For setups running more than one Hermes agent identity (e.g. two Discord bots
with different personas/models that chat with each other or split work).

**Skills are scoped per Hermes profile, not per model/ACP CLI.** `default`
→ `$HERMES_HOME/skills/`. A named profile → `~/.hermes/profiles/<name>/skills/`.
A skill only fires for the profile it's installed in.

**ACP backends (Cursor, Claude Code CLI, etc.) only lend the model** — they
have no skill storage of their own. `~/.cursor/skills` is Cursor-IDE-only and
invisible to a Hermes profile that merely borrows Cursor as its ACP backend.

Want the same skill (house tone rules, a shared workflow) live in two
profiles? **Copy it into both `skills/` trees** — there is no shared/global
skill store across profiles.

**Cron jobs are also scoped per profile.** `cronjob list` (or `hermes cron
list`) only shows the current profile's jobs; a job created under `default`
is invisible to a `claude` profile and vice versa.

#### Moving a cron job between profiles

There's no "move" operation — a job's `job_id` belongs to that profile's job
DB. To relocate one:

1. Read the full prompt/schedule/deliver/toolsets/workdir of the source job
   before touching anything.
2. Point CLI calls at the target profile (e.g. set `HERMES_HOME` to that
   profile's directory, or use the profile-scoped API/`profile` param) and
   re-create the job there with `cronjob(action='create', ...)`.
3. If the job references another via `context_from` (e.g. a morning-briefing
   job that reads yesterday's EOD-review output), **re-link `context_from` to
   the new job's id** — ids are assigned at creation time, so the old id is
   now dangling once you recreate.
4. For `script:`-backed (`no_agent`) jobs, copy the script file itself into
   the target profile's `scripts/` folder too — scripts aren't shared any more
   than skills are.
5. Only remove the original job from the source profile after confirming the
   new one exists and is enabled in the target profile.

**Pitfall:** Don't assume "install a skill / schedule a cron once" covers
every bot in a multi-agent setup — ask which profile(s) actually need it
before duplicating. Blanket-copying bloats every profile's system prompt for
no benefit.
