...
5. **probe 에이전트 bash 경로 (Windows 치명적)** — `agents/probe/coral-agent.toml` 의 `path = "bash"` 는 mac/linux 에선 정상이나, **Windows 예약작업/서비스 컨텍스트에서 `bash` 가 WSL 로 풀려 `execvpe(/bin/bash) failed` → probe 5개 즉시 사망 → 세션 close (probe URL 미기록)**. Windows 상시화 시 probe 런타임 경로를 **git-bash 절대경로**로 고정해야 한다. → **`install-coral.sh` 가 Windows(MINGW/MSYS/CYGWIN) 감지 시 `coral-agent.toml` 의 `path="bash"` 를 `cygpath -m` 로 산출한 git-bash 절대경로로 자동 치환**(2026-08-21, 멱등). (대화형 셸에선 우연히 되므로 오진 쉽다.)
6. **toolsets 누락 함정** — `setup-hermes-coral.sh`/`upsert_coral_mcp.py` 는 `mcp_servers.coral` 블록만 주입하고 **`toolsets` 에 `coral` 을 넣지 않는다**. toolset 에 `coral` 이 없으면 `mcp test` 는 Connected 라도 에이전트에게 `mcp_coral_*` 가 **노출되지 않아 무전 불가**. → **`upsert_coral_mcp.py` 가 `mcp_servers.coral` 주입 시 `toolsets` 에 `coral` 도 멱등 추가**하도록 수정됨(2026-08-21). 별도 수동 조치 불필요.
...
11. **헬스 워치독(cron) — `.sh` 아닌 `.py` 로** — 서버/세션 상시 감시는 `hermes/coral/coral_health.py.template` 을 쓴다.
    > ⚠️ **hermes cron 은 HERMES_HOME(=프로필)별로 분리된다.** 잡은 `<HERMES_HOME>/cron/jobs.json` 에 저장되고 **그 HERMES_HOME 의 게이트웨이 ticker 가 있어야 발화**한다. 5인조는 프로필 게이트웨이(`--profile <p>`)만 상시 가동하고 **root(무프로필) 게이트웨이는 보통 안 뜨므로, root `cron/` 에 넣은 잡은 발화하지 않는다**. 따라서 프로필 전용 잡(coral-health=ops 등)은 반드시 **ticker 가 살아있는 해당 프로필** 밑에 등록할 것. "모든 cron 을 한 폴더(root)로" 는 root 게이트웨이를 서비스로 상시화하지 않는 한 동작하지 않는다. **반드시 Python** 이어야 한다: 스케줄러는 `.sh` 를 PATH `bash`(게이트웨이 Task Scheduler 컨텍스트에선 WSL)로 돌려 `execvpe(/bin/bash)` 로 죽지만, `.py` 는 스케줄러 내장 Python 으로 실행되어 안전하다. 정상 시 무출력(=무전송), 서버/세션이 죽었을 때만 `setup-hermes-coral.sh`(git-bash 절대경로 호출) 로 재생성·재주입. 설치:
    ```bash
