...
|----------|--------|--------------------------|---------------------|---------------|
| **마늘쫑쿵야** | PM / 아키텍트 (요구사항 분해·배정) | `pm` | `pm` | 1534806088762003476 |
| **양파쿵야** | **개발 리드** (전반적 개발 구현) | `dev` | `dev` | 1513699678556782703 |
| **무시쿵야** | **인프라 개발자** (인프라 설정/구축/배포) | `infra` | `infra` | 1535078180077965354 |
| **샐러리쿵야** | QA 수문장 (검수 PASS/REJECT) | `qa` | `qa` | 1513768982472032266 |
| **버섯쿵야** | 비서 / 보고 (칸반 모니터링·디스패처) | `ops` | `ops` | 1526011878281842728 |
...
> - **양파쿵야**: 기존(인프라/API/터미널) → **개발 리드** (전반적 개발 구현 담당)
...
대장님 지시 (Discord #work)
   ↓
[마늘쫑/pm] 요구사항 분석 → 칸반 카드 분해 + 담당 배정
   ↓
[양파/dev] 개발 리드 — 실제 코딩 구현
   ↓
[무시/infra] 인프라 개발자 — 인프라 설정/구축/배포
   ↓
[샐러리/qa] 검수
   PASS  → 칸반 Done + 버섯에게 이관
   REJECT → 담당자에게 수정 요구 + In Progress 복귀
   ↓
[버섯/ops] 칸반 모니터링(유일 디스패처) → 대장님 보고
```
...
| `pm` (마늘쫑) | false | (미사용) | spawn / 분해 |
| `dev` (양파) | false | (미사용) | spawn only |
| `infra` (무시) | false | (미사용) | spawn only |
| `qa` (샐러리) | false | (미사용) | spawn only |
...
| 템플릿 | 대상 프로필 | 페르소나 |
|---------|-----------|----------|
| `profiles/pm/SOUL.md.template` | `pm` | 마늘쫑 (PM) |
| `profiles/dev/SOUL.md.template` | `dev` | 양파 (개발 리드) |
| `profiles/infra/SOUL.md.template` | `infra` | 무시 (인프라) |
| `profiles/qa/SOUL.md.template` | `qa` | 샐러리 (QA) |
| `profiles/ops/SOUL.md.template` | `ops` | 버섯 (비서) |
